...
- **ASL recognizer: TRAIN one small landmark classifier for A–Z + 0–9 (CONFIRMED — plan of record).** We do NOT train any neural net from scratch and we do NOT train the hand detector — MediaPipe Hands is reused as-is. We train only a **small classifier head** (an `scikit-learn` SVM/RandomForest or a 2-layer MLP) over MediaPipe's 21 normalized landmarks — minutes of CPU training on a CSV of landmark vectors.
  - **Dataset (chosen):** merge two ready-made **landmark** datasets — [srisahithis A–Z + Hand Landmarks](https://www.kaggle.com/datasets/srisahithis/american-sign-language-a-z-dataset-hand-landmarks) (letters) + [rayeed045 ASL Digit Dataset](https://www.kaggle.com/datasets/rayeed045/american-sign-language-digit-dataset) (numbers) — both are already in the 21-keypoint format, so almost no preprocessing. These are **sufficient to train on their own — no self-recorded data is required.** (Optional accuracy tuning only, if the model struggles with your camera: capture a few of your own samples per sign later, or add an in-app per-user calibration step.) **Single-source alternative:** the [36-class comprehensive set](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC12877850/) (run MediaPipe over the images once). See the dataset list below.
  - **Training notebook (ready):** `train_asl_landmarks.ipynb` (in this change folder) — a Google Colab notebook that downloads both Kaggle datasets, extracts + normalizes MediaPipe landmarks, trains the classifier (SVM/RandomForest), and exports `asl_landmark_model.joblib` + `labels.json`. CPU-only, ~10–20 min, no self-recorded data needed.
  - **Fallback:** a landmark **KNN/template matcher** needs zero training and is the offline / quick-start fallback.
  - Scope note: this "small classifier" does NOT extend to Feature 4 (see Architecture § B; that remains unsolved and out of scope).
...
**CHOSEN (least effort):** merge the two **landmark** datasets — **srisahithis** (A–Z) + **rayeed045** (0–9) — since both are already in MediaPipe 21-keypoint form, then train the small classifier. These public sets are enough on their own; **self-recorded samples are optional** (only for later accuracy tuning / per-user calibration). **Single-source alternative:** the **36-class comprehensive set** (run MediaPipe over the images once); **grassknoted** can bulk up letters if needed. ⚠️ Skip British (BSL) and Turkish "sign digits" sets for labels — handshapes differ from ASL. ⚠️ Note ASL letter/number **handshape overlaps** (e.g. digit "2"≈"V", "6"≈"W", "9"≈"F", "0"≈"O"); disambiguate by mode (the app knows whether it is expecting a letter or a digit).
